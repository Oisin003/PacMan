public class Clyde extends Ghost implements Comparable<Location>
{
  //private int x;
  //private int y;
  ...
  public Clyde(Location location)
  {
    super(location);
    int xy[] = location.getXY();
    this.x=xy[0];
    this.y=xy[1];
    this.colour=Game.ORANGE;
    this.direction=1;
  ...
  @Override
    ...
    int i = Integer.compare(getLocation().getX(), otherLocation.getX());
    if (i == 0) {
      i = Integer.compare(getLocation().getY(), otherLocation.getY());
    }
    return i;
  }
  ...
  @Override
    ...
    return "[Clyde:location=" + location.toString() + ", direction=" + direction + ", speed=" + speed + ", moving=" + moving + ", bearing=" + bearing + "]";
  }
